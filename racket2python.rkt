...
(define-syntax-rule (deffilm . body) (python '(deffilm . body)))
(define-syntax-rule (film name . body) (python '(film name . body)))
...
(define-syntax-rule (pylet . body) (python-statement '(begin (let . body) (void))))
(define-syntax-rule (pydefine . body) (python-statement '(begin (define . body) (void))))
(define-syntax-rule (py: . body) (void))
(define-syntax-rule (py#%app . body) (python-statement '(begin body (void))))
(define-syntax-rule (pyset! . body) (python-statement '(begin (set! . body) (void))))
...
(define (python-format-str str (i 0))
...
(define (unparse-parameter param)
  (match param
    (`(,param : ,type ,init)
     (display (python-name param))
     (display "=")
     (python-expression init)
     param)
    (`(,param : ,type)
     (display (python-name param))
     param)
...
(define (python-parameters params)
  (cond ((null? params)
         (list))
        (else
...
(define (python-statement stmt [indent 0] [return? #f])
  (match stmt
    ...
    ((or `(define #:forall ,_ (,name . ,params) : ,_ . ,body)
     ...
     (advance indent)
     (printf "def ~A(" (python-name name))
     (let ((params
            (if (symbol? params)
                (begin
                  (printf "*~A" (python-name params))
                  (list params))
    ...
    (`(when ,test . ,stmts)
     ...
     (printf ":~%")
     (python-statements stmts (+ indent 4)))
    (`(unless ,test . ,stmts)
     (advance indent)
     (printf "if ")
     (python-expression `(not ,test))
     (printf ":~%")
     (python-statements stmts (+ indent 4)))
    (`(set! ,name ,init)
     (advance indent)
     (python-expression name)
     (printf " = " )
     (python-expression init))
    ((or `(let ,(? symbol? rec) : ,_ ,bindings . ,stmts)
     ...
     (`(let ,bindings . ,stmts)
      (if (null? bindings)
         (python-statements stmts indent)
         (begin
           (advance indent)
           (let ((vars (map binding-var bindings))
                 (vals (map binding-val bindings)))
             (python-parameters vars)
             (printf " = ")
             (python-expressions vals))
           (newline)
           (python-statements stmts indent return?))))
    (`(let* ,bindings . ,stmts)
    ...
    (`(parameterize ,bindings . ,stmts)
     (python-statement `(let ,bindings . ,stmts) indent))
    (`(if ,test ,conseq ,altern)
     (advance indent)
     (printf "if ")
     (python-expression test)
     (printf ":~%")
     (python-statement conseq (+ indent 4) return?)
     (newline)
     (advance indent)
     (printf "else:~%")
     (python-statement altern (+ indent 4) return?))
    (`(cond ,clause . ,clauses)
     (advance indent)
     (match clause
       (`(,test . ,stmts)
    ...
    (`(begin ,stmt . ,stmts)
     ...
     (python-statements stmts indent return?))
    (`(begin0 ,stmt . ,stmts)
     (python-statement `(let ((res ,stmt)) ,@stmts res) indent return?))
    (`(for-each ,f ,l)
     (python-statement `(for ([e ,l]) (,f e)) indent return?))
    (`(pass)
     (advance indent)
     (printf "pass"))
    ((or `(for : ,_ ([,var : ,_ ,expr]) . ,body)
     ...
     (printf "for ")
     (if (cons? var)
         (python-expressions var)
         (python-expression var))
     (printf " in ")
     (if (number? expr)
       (python-expression `(range ,expr))
       (python-expression expr))
     (printf ":~%")
     (python-statements body (+ indent 4)))
     ...
     (let ((params (for/list ((param params))
           ...
           (inits (for/list ((param params))
                     (match param
                       ((or `(,param : ,_ ,init) `(,param ,init))
                        init)))))
       (when (ormap (lambda (param) (member (python-name param) (locals))) params)
         (error "Repeated variable" params))
       (python-statement `(for ((,params (zip . ,inits))) . ,body) indent return?)))
    (`(with ,expr . ,stmts)
     (advance indent)
     (printf "with ")
     (python-expression expr)
    ...
    (`(z-rotating ,e . ,body)
     (python-statement
      ...
      indent
      return?))
    (`(with-current-layer ,expr . ,stmts)
     (advance indent)
     (printf "with current_layer(")
     (python-expression expr)
     (printf "):~%")
     (python-statements stmts (+ indent 4)))
    (`(deftikz . ,body)
     (python `(defmodel . ,body) indent))
    (`(defmodel . ,body)
     (python `(defmodel . ,body) indent))
    (`(deffilm . ,body)
     (python `(deffilm . ,body) indent))
    (`(film . ,body)
     (python `(film . ,body) indent))
    (`(pynewline)
     (newline))
    (else
     (match stmt
       (`(error . ,_)
        (python-maybe-indent-expression "raise " stmt indent))
       (else
        (python-maybe-indent-expression (if return? "return " "") stmt indent))))))


(define (python-statements stmts indent [return? #f])
  (if (null? stmts)
      (python-statement `(pass) indent)
      (let ((first? #t))
        ...
        (python-statement (last stmts) indent return?))))
...
(define python-line-limit 80)
(define break-line? (make-parameter #f))

(define (python-maybe-indent-expression start expr indent)
  (let ((so (open-output-string)))
    ...
    (let ((str (get-output-string so)))
      (if (> (+ (string-length str) indent) python-line-limit)
          (let ((so (open-output-string)))
            (port-count-lines! so)
            (parameterize ((current-output-port so)
                           (break-line? #t))
...
(define (python-call fn args)
  (python-expression fn (operator-precedence 'call))
  (display "(")
  (cond ((null? args)
         )
        ((break-line?)
         (let-values (((line column pos) (port-next-location (current-output-port))))
           (parameterize ((break-line? #f)) (python-expression (first args)))
           (for ((arg (rest args)))
             (display ",")
             (newline)
             (advance column)
             (parameterize ((break-line? #f)) (python-expression arg)))))
        (else
         (python-expressions args)))
  (display ")"))
...
(define (python-expression e [precedence 0])
  ...
  (match e
    ...
    (`(splice-arg ,arg)
     (display "*")
     (python-expression arg))
    (`(apply ,fn . ,args)
     (rec
      `(,fn ,@(drop-right args 1) (splice-arg ,(last args)))))
    (`(if ,test ,conseq ,altern)
     (maybe-paren ('if op-precedence precedence)
       (python-expression conseq (+ 0.5 op-precedence))
       (display " if ")
       (python-expression test (+ 0.5 op-precedence))
       (display " else ")
       (python-expression altern (+ 0.5 op-precedence))))
    (`(cond (else ,conseq))
     (rec conseq))
    ...
    (`(index ,lst ,n)
     (maybe-paren ('index op-precedence precedence)
       (python-expression lst op-precedence))
     (printf "[")
     (python-expression n)
     (printf "]"))
    (`(list-range ,from ,to)
     (when from
       (python-expression from))
     (printf ":")
     (when to
       (python-expression to)))
    (`(drop ,lst ,n)
     ...
     (let ((unary-op (dict-ref '((+ . unary+) (- . unary-) (~ . unary~)) op op)))
       (maybe-paren (unary-op op-precedence precedence)
         (let ((space? (member op '(not))))
           (display op)
           (when space? (display " "))
           (python-expression arg op-precedence)))))
    (`(sqr ,x)
     (rec `(expt ,x 2)))
    (`(/ ,x -2)
     (rec `(- (/ ,x 2))))
...
#;    ((or `(for/list : ,_ (,(or `(,var0 ,init0) `(,var0 : ,_ ,init0))) (let (,(or `(,var1 ,init1) `(,var1 : ,_ ,init1))) ,body))
                        ...
                        (append (renamed-variables)
                                (map (lambda (var)
                                     ...
                                     (filter (lambda (var)
                                             ...
                                             vars)))))
         (printf "[")
         (python-expression body)
         (printf " for ")
         (python-expressions vars)
         (printf " in "))
       (python-expression expr)
       (printf "]")))
    ...
    (`(format ,str . ,args)
     (print (python-format-str str))
     (printf ".format(")
     (python-expressions args)
     (printf ")"))
    (`(attribute ,obj ,expr)
     (python-expression obj (operator-precedence 'attribute))
     (printf ".")
     (python-expression expr))
    (`(,fn . ,args)
     (python-call fn args))
    ...
    ((? symbol? x)
     (display (python-name x)))))

(define (python-expressions args)
  (cond ((null? args))
        (else
         (python-expression (car args))
         (for ((arg (cdr args)))
           (display ", ")
           (python-expression arg)))))

(define (python e [indent 0])
  (match e
    (`(defmodel (,name) . ,stmts)
     (printf "class test_~A(model):~%" (python-name name))
     (printf "    def create(self):~%")
     (python-statements stmts 8)
     (newline)
     (newline))
    (`(deffilm (,name) . ,stmts)
     (printf "class test_~A(film):~%" (python-name name))
     (printf "    def create(self):~%")
     (python-statements stmts 8)
     (newline)
     (newline))
    (`(film . ,body)
     (unparsable `(film . ,body)))
    (`(define . ,body)
     (python-statement `(define . ,body)))
    ;; (`(def-shape (,name . ,params) . ,stmts)
    ;;  (printf "class ~A(shape):~%" (python-name name))
    ;;  (python-statement `(define (create self . ,params) . ,stmts) 4))
    ;; (`(def-shape* (,name [,param : ,_type *]) . ,stmts)
    ;;  (printf "class ~A(shape):~%" (python-name name))
    ;;  (python-statement `(define (create self ,param) . ,stmts) 4))
    (`(def-shape (,name . ,params) . ,stmts)
     (printf "@shape_constructor(shape)~%")
     (python-statement `(define (,name . ,params) . ,stmts)))
    (`(def-shape* (,name [,param : ,_type *]) . ,stmts)
     (printf "@shape_constructor(shape)~%")
     (python-statement `(define (,name . ,param) (let ((pts (unvarargs ,param))) . ,stmts))))
    (`(: . ,_) ;type declaration
     #t)
    (`(unless-backends . ,_)
     #t)
    ...
    (`(define-syntax-rule . ,_)
     #t)
    ((or `(require (planet aml/rosetta))
         `(require rosetta))
     (printf "from Khepri import *~%"))
    (`(struct ,name ,fields)
     (printf "class ~A:~%" (python-name name))
     (let ((fields
            (map (lambda (field)
                   (match field
                     (`(,name : ,type) name)
                     (else name)))
                 fields)))
       (python-statement `(define (__init__ self ,@fields)
                            ,@(map (lambda (field)
                                     `(set! (attribute self ,field) ,field))
                                   fields))
                         4)))
    (else
     (python-statement e indent))))

(define-syntax-rule (py form ...) (begin (begin (python 'form) (newline)) ...))
